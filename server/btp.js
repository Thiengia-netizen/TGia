...
const { reconcileBtpNvl, markBtpGiaSanXuat, filterBtpSecrets } = require('./btp-nvl-sync');

const VALID_BRANCHES = ['khapkhun'];
function validBranch(b) { return VALID_BRANCHES.includes(b); }

// 🔒 CHỐT CHẶN THẬT SỰ Ở SERVER — không chỉ ẩn giao diện: mọi route đụng tới
// công thức/số liệu BTP của 1 chi nhánh đều phải qua middleware này. Luôn tra
// LẠI DATABASE (không tin dữ liệu client gửi lên) để biết user hiện có nằm
// trong S.user_btp_tabs của chi nhánh đó không. Super Admin luôn được qua.
async function requireBtpAccess(req, res, next) {
  ...
  try {
    ...
    const perm = data.user_btp_tabs?.[req.user.id] || [];
    if (perm.includes('btp-' + branch)) return next();
    return res.status(403).json({ success: false, message: 'Bạn chưa được Super Admin cấp quyền truy cập file BTP chi nhánh này.' });
  } catch (e) {
...
module.exports = function (io) {
  ...
  async function saveState(client, data, updatedBy, senderSocketId) {
    ...
    // 🔒 Cũng phải lọc per-recipient ở đây — route trong file này chính là nơi
    // TẠO/SỬA/XOÁ công thức BTP, nên càng phải cẩn thận không phát nguyên bản
    // đầy đủ cho những client không có quyền xem chi nhánh đó.
    io.sockets.sockets.forEach((s) => {
      if (s.id !== senderSocketId) {
        const filteredForThisUser = filterBtpSecrets(data, s.user);
        s.emit('state-updated', { data: filteredForThisUser, updated_at, updated_by: updatedBy });
      }
    });
    return updated_at;
  }
  ...
  router.post('/secure-check', requireAuth, requireBtpAccess, async (req, res) => {
  ...
  router.post('/change-branch-password', requireAuth, requireBtpAccess, async (req, res) => {
  ...
  router.get('/recipes', requireAuth, requireBtpAccess, async (req, res) => {
  ...
  router.post('/save-dish', requireAuth, requireBtpAccess, async (req, res) => {
 ...
 router.delete('/dish/:id', requireAuth, requireBtpAccess, async (req, res) => {
    const { branch } = req.query;
    if (!validBranch(branch)) {
      return res.status(400).json({ success: false, message: 'Chi nhánh không hợp lệ!' });
    }
  ...
  router.post('/produce', requireAuth, requireBtpAccess, async (req, res) => {
